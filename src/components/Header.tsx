      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-2 cursor-pointer" onClick={() => navigate('/')}>
            ...
            <h1 className="text-2xl font-display font-bold wedding-text-gradient">Wellington County Wedding Guide</h1>
          </div>

          {/* Desktop Navigation */}
          <nav className="hidden md:flex items-center space-x-8">
            <button onClick={() => navigate('/')} className={`transition-colors ${isVendorPage ? 'text-wedding-pink' : 'text-gray-700 hover:text-wedding-pink'}`}>
              Find Vendors
            </button>
            <button onClick={() => navigate('/budget')} className={`flex items-center gap-2 transition-colors ${isBudgetPage ? 'text-wedding-pink' : 'text-gray-700 hover:text-wedding-pink'}`}>
          ...
          {isVendorPage && onSearch && <div className="hidden md:flex items-center space-x-4 flex-1 max-w-md ml-8">
              <div className="relative flex-1">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
                <Input type="text" placeholder="Search vendors..." value={searchQuery} onChange={e => onSearch(e.target.value)} className="pl-10 pr-4 py-2 w-full border-gray-200 focus:border-wedding-pink focus:ring-wedding-pink" />
              </div>
            </div>}
        ...
        {isVendorPage && onSearch && <div className="md:hidden pb-4">
