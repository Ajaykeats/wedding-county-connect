  ...
  const filteredVendors = useMemo(() => {
    return vendors.filter(vendor => {
      ...
      if (searchQuery) {
        ...
        const matchesSearch = vendor.name.toLowerCase().includes(query) || vendor.description.toLowerCase().includes(query) || vendor.services.some(service => service.toLowerCase().includes(query));
  ...
  return <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-6xl font-display font-bold wedding-text-gradient mb-4">Find Your Perfect Wedding Services</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredVendors.map(vendor => <VendorCard key={vendor.id} vendor={vendor} onClick={() => setSelectedVendor(vendor)} />)}
        </div>
      ...
      {/* Vendor Modal */}
      <VendorModal vendor={selectedVendor} isOpen={!!selectedVendor} onClose={() => setSelectedVendor(null)} />
    </div>;
};
